...
type Props = {
  ...
  children: (filteredAndSortedJobs: WorkJob[]) => React.ReactNode;
};
...
export default function WorkFilters({
  ...
  const [pendingCompanies, setPendingCompanies] = useState<Set<string>>(selectedCompanies);

  const companiesWithCount = jobs.reduce<{ company: string; count: number }[]>((acc, job) => {
  ...
  const handleFilterToggle = (company: string) => {
  ...
  const handleApplyFilter = () => {
    onCompaniesChange(pendingCompanies);
    setFilterOpen(false);
  };

  const handleClearFilter = () => {
    setPendingCompanies(new Set());
    onCompaniesChange(new Set());
    setFilterOpen(false);
  };

  const totalCompanies = companiesWithCount.length;
  const nonSelectedCount = totalCompanies - selectedCompanies.size;
  const activeFilterCount = selectedCompanies.size;
  const filtered =
    activeFilterCount === 0
      ? jobs
      : jobs.filter((job) => selectedCompanies.has(job.company));
  const sorted =
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 w-full">
        ...
        <div className="relative inline-block" ref={filterRef}>
          <button
            ...
            onClick={() => {
              setFilterOpen((o) => !o);
              if (!filterOpen) setPendingCompanies(selectedCompanies);
              setSortOpen(false);
            }}
          ...
          {filterOpen && (
            <div className="absolute left-0 top-full z-20 mt-2 min-w-[220px] max-h-[280px] flex flex-col rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-lg overflow-hidden">
              <div className="overflow-y-auto p-2 flex-1">
                {companiesWithCount.map(({ company, count }) => (
                  <label
                    key={company}
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={pendingCompanies.has(company)}
        ...
        <div className="relative" ref={sortRef}>
          ...
          {sortOpen && (
            <div className="absolute left-0 top-full z-20 mt-2 min-w-[180px] rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-lg overflow-hidden py-1">
              <button
                type="button"
                onClick={() => {
